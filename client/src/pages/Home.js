...
class Home extends Component {
  ...
  submitSearch = event => {
    ...
    API.getArticles(topic, startYear, endYear).then(res => {
          ...
          <div className="panel-body container">
            <form id="form" onSubmit={this.submitSearch}>
              ...
              <div className="row">
                <div className="form-group">
                  <input
                    className="btn btn-success col-xs-offset-2 col-xs-8"
                    type="submit"
                    value="Search"
                  />
                </div>
              ...
              this.state.articles.map(a => (
                <div className="panel panel-default" key={a._id}>
                  <a target="_blank" href={a.web_url}>
                    <h4>{a.headline.main}</h4>
                  </a>
                  <h5>{a.pub_date}</h5>
                  <h5>
                    <a href={a.web_url} target="_blank">
                      {a.web_url}
                    </a>
                  </h5>
                  <button
